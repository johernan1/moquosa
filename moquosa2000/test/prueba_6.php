...
echo "joint_loads_force_SOL\n";
print_r($joint_loads_force_SOL);
echo "joint_loads_force_RES\n";
print_r($joint_loads_force_RES);

$joint_loads_force_normalized_SOL = $procesador->normalizar_joint_loads_force($joint_loads_force_SOL, $joint_coordinates_normalized_SOL, $unidadesL_SOL, $unidadesF_SOL, $EscM, $EscF);
$joint_loads_force_normalized_RES = $procesador->normalizar_joint_loads_force($joint_loads_force_RES, $joint_coordinates_normalized_RES, $unidadesL_RES, $unidadesF_RES);

$procesador->chk_joint_loads($joint_loads_force_normalized_SOL, $joint_loads_force_normalized_RES, $DOF_SOL, $f1);
fwrite($f, $f1->val_sin_br()); $f1->val='';
}
...
$frame_loads_distributed_SOL = $procesador->buscar_patron_SAP_tokens($archivoSOL, $patron, $procesador->tokens_frame_loads_distributed);
$frame_loads_distributed_RES = $procesador->buscar_patron_SAP_tokens($archivoRES, $patron, $procesador->tokens_frame_loads_distributed);

echo "frame_loads_distributed_SOL\n";
$procesador->printL($frame_loads_distributed_SOL);
echo "frame_loads_distributed_RES\n";
$procesador->printL($frame_loads_distributed_RES);

$frame_loads_distributed_normalized_SOL = $procesador->normalizar_frame_loads_distributed($frame_loads_distributed_SOL, $frame_connectivity_normalized_SOL, $unidadesL_SOL, $unidadesF_SOL, $EscM, $EscF);
$frame_loads_distributed_normalized_RES = $procesador->normalizar_frame_loads_distributed($frame_loads_distributed_RES, $frame_connectivity_normalized_RES, $unidadesL_RES, $unidadesF_RES, 1, 1);


//echo "frame_loads_distributed_normalized_SOL\n";
//$procesador->printL($frame_loads_distributed_normalized_SOL);
echo "prueb_5: frame_loads_distributed_normalized_RES\n";
$procesador->printL($frame_loads_distributed_normalized_RES);
echo ("-----\n");
$procesador->printL($frame_loads_distributed_normalized_SOL);

//exit(1);

$procesador->chk_frame_loads_distributed($frame_loads_distributed_normalized_SOL,
                                         $frame_loads_distributed_normalized_RES,
                                         $procesador->tokens_frame_loads_distributed_normalized,
                                         [$procesador,'obtener_resultante_frame_loads_distributed'],
                                         $DOF_SOL, $f1);
fwrite($f, $f1->val_sin_br()); $f1->val='';

//-------------------------------------------------------------------------------
fwrite($f, "----------------------------------------------------------------\n");
fwrite($f, "CHEQUEO CARGAS PUNTUALES EN BARRAS\n");
fwrite($f, "----------------------------------------------------------------\n");

$patron = '/Frame.*LoadPat.*Type=Force.*Force/';

// Buscar el patrón en el archivo SOL y RES para obtener las cargas puntuales
$frame_loads_point_SOL = $procesador->buscar_patron_SAP_tokens($archivoSOL, $patron, $procesador->tokens_frame_loads_point);
$frame_loads_point_RES = $procesador->buscar_patron_SAP_tokens($archivoRES, $patron, $procesador->tokens_frame_loads_point);

// Imprimir resultados para verificar
$procesador->printL($frame_loads_point_SOL, "frame_loads_point_SOL");

// Normalizar las cargas puntuales obtenidas del archivo SOL
$frame_loads_point_normalized_SOL = $procesador->normalizar_frame_loads_point(
    $frame_loads_point_SOL, $frame_connectivity_normalized_SOL,
    $unidadesL_SOL, $unidadesF_SOL, $EscM, $EscF
);

// Normalizar las cargas puntuales obtenidas del archivo RES
$frame_loads_point_normalized_RES = $procesador->normalizar_frame_loads_point(
    $frame_loads_point_RES, $frame_connectivity_normalized_RES,
    $unidadesL_RES, $unidadesF_RES, 1, 1
);

// Imprimir resultados normalizados para verificar
$procesador->printL($frame_loads_point_normalized_SOL, "frame_loads_point_normalized_SOL");

// Comprobar las cargas distribuidas en los patrones normalizados
$procesador->chk_frame_loads_distributed(
    $frame_loads_point_normalized_SOL,
    $frame_loads_point_normalized_RES,
    $procesador->tokens_frame_loads_point_normalized,
    [$procesador,'obtener_resultante_frame_loads_point'],
    $DOF_SOL, $f1
);
fwrite($f, $f1->val_sin_br()); $f1->val='';
